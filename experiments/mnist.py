...
def load_mnist(batch_size: int, train: bool, subset_size=None, shuffle: bool = True) -> DataLoader:
...
def train_model(save_path: Path, device: torch.device, n_epoch: int = 10, batch_size_train: int = 64,
                batch_size_test: int = 1000, random_seed: int = 42, learning_rate: float = 0.01,
                momentum: float = 0.5, log_interval: int = 100, model_reg_factor: float = 0.01, cv: int = 0) \
        -> MnistClassifier:
...
def fit_explainers(device: torch.device, explainers_name: list, save_path: Path, corpus_size: int = 1000,
...
def fit_representer(model_reg_factor: float, load_path: Path, cv: int = 0) -> Representer:
